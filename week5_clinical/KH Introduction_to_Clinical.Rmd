_---
title: "Intro to TCGA Clinical Data COMPLETE"
date: 07/17/22
updated: 09/26/24
---
...
```{r}
write.csv(clinic, "/home1/kehuffma/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/kehuffma/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
#    typeof(clinic$gender) = character
# 3. The age in years of the 5th patient (barcode: TCGA-4H-AAAK) is 50      years old. 
# 4. The 1st patient was 65 years old at their last follow up (barcode:   `  TCGA-3C-AAAU)? 
...
```{r Exercise 3.3}
# write code here
# notation 1: clinic[, "vital_status"] 
# notation 2:
clinic[, 14]
...
```{r Exercise 4.1}
# create plot here
# Choose two variables: age_at_diagnosis and lymph_nodes_examined_count
# Types of Variables: Continuous and discrete variables can be used in scatterplots, but        mainly continuous variables are used
# Extra Challenge:
plot(clinic$age_at_diagnosis, clinic$lymph_nodes_examined_count,
     main = "Scatter Plot of Age at Diagnosis vs. Lymph Nodes Examined Count", # plot title
     xlab = "Age at Diagnosis", # x-axis title
     ylab = "Lymph Nodes Examined Count", # y-axis title
...
```{r Exercise 4.2}
# create plot here
# Two Variables: age_at_diagnosis and lymph_nodes_examined_count
# I was having issues getting the other variables to work for the box plot even though they were also categorical/numerical variables. So, I had to reuse variables from the previous section. 
# Types of variables: you can use categorical and continuous variables in a box plot
# usually categorical on the x-axis and cts on the y-axis

clinic$age_at_diagnosis <- as.factor(clinic$age_at_diagnosis)

boxplot(clinic$age_at_diagnosis ~ clinic$lymph_nodes_examined_count,
        main = "Box Plot of Lymph Nodes Count by Age",  # title of the plot
        xlab = "Age", # x-axis label
        ylab = "Lymph Nodes Count", # y-axis label
        col = c("lightblue", "lightgreen"), # colors for the boxes
        notch = FALSE) # toched boxes for confidence intervals
...
```{r}
# check if there are any NA values in age_at_diagnosis
na_exists <- any(is.na(cleaned_clinic$age_at_diagnosis))
print(na_exists)
...
if (any(is.na(cleaned_clinic$survival_time))) {
  warning("NAs introduced in survival_time conversion.")
}
...
```{r}
# 1. The y axis represents the survival probability. 

# 2. The x-axis represents time. 

# 3. The middle-aged group appears to have the greatest rate of survival.

# 4. The young age group appears to have the worst rate of survival.  

# 5. The approx. 15-year survival rate for "Old" breast cancer patients is 0.25 or 25%.

# 6. The p-value is 0.2. This p value is not statistically significant as it is not less than or equal to p = 0.05. This tells us the relationship between age and breast cancer survival is not statistically significant based on the data.
```
